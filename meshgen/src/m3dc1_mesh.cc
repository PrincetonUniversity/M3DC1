...
//*******************************************************
{
  ...
  if (!(m3dc1_mesh::instance()->field_container->count(NODE_GLB_ORDER)))
  {
    ...
    if (!PCU_Comm_Self()) std::cout<<"[M3D-C1 INFO] "<<__func__<<": create numbering "<<getName(n)<<"\n";
...
// **********************************************
bool is_ent_owned(Mesh2* mesh, MeshEntity* e)
// **********************************************
{
...
// **********************************************
{
  ...
  {
    double id[1];
    apf::Field* f = m3dc1_ghost::instance()->mesh->findField("node global id field");
    apf::getComponents(f, e, 0, id);
    if (m3dc1_ghost::instance()->org_node_flag->at((int)id[0]) == true)
      return false;
  }
  else //if (ent_dim==m->getDimension())
  {
    if (get_ent_ownpartid(m, e)== PCU_Comm_Self())
      return false;
  }
  return true;
}
...
// **********************************************
int get_ent_ownpartid(Mesh2* m, MeshEntity* e)
// **********************************************
{
  int own_partid;
  if (!m3dc1_ghost::instance()->num_local_ent[0])
  ...
  {
    double partid[1];
    apf::Field* own_f=m->findField("node own partid field");
...
// *********************************************************
int get_ent_globalid (Mesh2* m, MeshEntity* e, int ent_dim)
// *********************************************************
{
  apf::Field* f;
  if (ent_dim==0) 
    f = m->findField("node global id field");
  else // (*ent_dim==m->getDimension()) 
    f = m->findField("elem global id field");
  double id[1];
  apf::getComponents(f, e, 0, id);
  return (int)id[0];
}
...
//    std::cout<<"[M3D-C1 INFO] (p"<<PCU_Comm_Self()<<") create PRISM with face "<<get_ent_localid(mesh, wedge_faces[0])<<"(t="<< mesh->getType(wedge_faces[0])<<"), "<<get_ent_localid(mesh, wedge_faces[1])<<"(t="<< mesh->getType(wedge_faces[1])<<"), "<<get_ent_localid(mesh, wedge_faces[2])<<"(t="<< mesh->getType(wedge_faces[2])<<"), "<<get_ent_localid(mesh, wedge_faces[3])<<"(t="<< mesh->getType(wedge_faces[3])<<"), "<<get_ent_localid(mesh, wedge_faces[4])<<"(t="<< mesh->getType(wedge_faces[4])<<") (#face="<<mesh->count(2)<<")"<<std::endl;
  ...
  n = createNumbering(mesh,name.c_str(),getShape(f),1);
  {
    ...
    while ((e = mesh->iterate(it)))
    {
      if (!is_ent_owned(mesh,e)) continue;
...
// **********************************************
{
  ...
  while ((e = mesh->iterate(it)))
  {
    ...
    {
      if (is_ent_owned(mesh, elements[i]))
        ++num_adj_node;
    }
...
// *********************************************************
void m3dc1_mesh::print(int opt /* reserved*/)
// *********************************************************
{
  // print global info
  if (!PCU_Comm_Self())
  {
    ...
    std::cout<<"# global:\t\tv "<<num_global_ent[0]<<", e "<< num_global_ent[1]
             <<", f "<<num_global_ent[2]<<", r "<<num_global_ent[3]<<"\n";
  }
  MPI_Barrier(MPI_COMM_WORLD);

  int nsize = PCU_Comm_Peers();
  int* vec_num_own_ent=new int[nsize*4];
  int* vec_num_local_ent=new int[nsize*4];

  MPI_Allgather(num_local_ent, 4, MPI_INT, vec_num_local_ent, 4, MPI_INT, MPI_COMM_WORLD);
  MPI_Allgather(num_own_ent, 4, MPI_INT, vec_num_own_ent, 4, MPI_INT, MPI_COMM_WORLD);

  int num_plane_local_ent[4];
  int num_plane_own_ent[4];
  for (int i=0; i<4; ++i)
  {
    num_plane_local_ent[i] = 0;
    num_plane_own_ent[i] = 0;
  }

  int size_proc_grp = PCU_Comm_Peers()/m3dc1_model::instance()->num_plane;
  int start_index=size_proc_grp*m3dc1_model::instance()->local_planeid*4;

  for (int i=start_index; i<start_index+size_proc_grp*4; ++i)
  {
    num_plane_local_ent[i%4] += vec_num_local_ent[i];
    num_plane_own_ent[i%4] += vec_num_own_ent[i];
  }

  // print plane info
  if (PCU_Comm_Self()%size_proc_grp==0) // master on a plane
      std::cout<<"* plane "<<m3dc1_model::instance()->local_planeid<<": \n"
               <<"\t# local:\tv "<<num_plane_local_ent[0]<<", e "<<num_plane_local_ent[1]
               <<", f "<<num_plane_local_ent[2]<<", r "<<num_plane_local_ent[3]<<"\n"
               <<"\t# owned:\tv "<<num_plane_own_ent[0]<<", e "<< num_plane_own_ent[1]
               <<", f "<<num_plane_own_ent[2]<<", r "<<num_plane_own_ent[3]<<"\n"
               <<"\t# non-owned:\tv "<<num_plane_local_ent[0]-num_plane_own_ent[0]
               <<", e "<<num_plane_local_ent[1]-num_plane_own_ent[1]
               <<", f "<<num_plane_local_ent[2]-num_plane_own_ent[2]
               <<", r "<<num_plane_local_ent[3]-num_plane_own_ent[3]<<"\n";
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  {
    if (PCU_Comm_Self()==i)
      std::cout<<"* part "<<PCU_Comm_Self()<<" (plane: "<<m3dc1_model::instance()->local_planeid<<"): \n"
               <<"\t# local:\tv "<<mesh->count(0)<<", e "<< mesh->count(1)
               <<", f "<<mesh->count(2)<<", r "<<mesh->count(3)<<"\n"
               <<"\t# owned:\tv "<<num_own_ent[0]<<", e "<< num_own_ent[1]
               <<", f "<<num_own_ent[2]<<", r "<<num_own_ent[3]<<"\n"
               <<"\t# non-owned:\tv "<<mesh->count(0)-num_own_ent[0]
               <<", e "<< mesh->count(1)-num_own_ent[1]
               <<", f "<<mesh->count(2)-num_own_ent[2]
               <<", r "<<mesh->count(3)-num_own_ent[3]<<"\n";
      MPI_Barrier(MPI_COMM_WORLD);
  } 
  delete [] vec_num_own_ent;
  delete [] vec_num_local_ent;
}
