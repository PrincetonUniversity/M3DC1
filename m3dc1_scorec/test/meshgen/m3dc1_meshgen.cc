...
//char simLic[128]="27001@license.pppl.gov:/usr/pppl/Simmetrix/simmodsuite.lic"; //.20170317";
char simLic[128]="/usr/pppl/Simmetrix/simmodsuite.lic";
//char simLic[128]="2800@v-cluster01.pppl.gov:/usr/pppl/Simmetrix/license.txt";
...
int main(int argc, char *argv[])
{
  ...
  while(EOF!=fscanf(infp,"%s",namebuff))
  {
    ...
    if(strcmp(namebuff,"inFile")==0) fscanf(infp, "%s", ptFile);
    if(strcmp(namebuff,"outFile")==0) fscanf(infp, "%s", modelName);
    if(strcmp(namebuff,"resistive-width")==0) fscanf(infp, "%lf", &thickness);
  ...
  if (modelType) useOriginal=0;
  else useVacuumParams=1;
...
// interpolate points
  if (modelType==1 ||modelType==3)
  {
    ...
    if(!fp) 
    {
      cout<<" unable to find file "<<ptFile<<endl;
      return 1;
    }
    fscanf(fp,"%d\n",&num_pts);
  ...
  if (modelType==3)
  {
    ...
    if (thickness>0)
    {
      ...
      cout<<"num pt merged "<<vtxMerged<<" num pt inserted "<< vtxInserted<<endl;
...
#include <iostream>
  ...
  if (modelType==3)
    sprintf(filename,"%s-%0.2f-%0.2f-%0.2f",modelName, thickness, width, height);
  else
  ...
  //SimLicense_start("geomsim_core,geomsim_adv,meshsim_core,meshsim_adv",simLic);
  ...
  printf("Number of mesh faces in surface: %d\n",M_numFaces(sim_mesh));

  sprintf(model_filename,"%s.txt",filename);
  cout<<"\n[m3dc1_meshgen INFO] model is saved in M3D-C1-readable \""<<model_filename<<"\"\n";

  sprintf(model_filename,"%s.smd",filename);
  cout<<"[m3dc1_meshgen INFO] model is saved in Simmetrix-readable \""<<model_filename<<"\"\n";

  sprintf(model_filename,"%s.dmg",filename);
  cout<<"[m3dc1_meshgen INFO] model is saved in PUMI-readable \""<<model_filename<<"\"\n";


if (M_numFaces(sim_mesh)>1000)
    sprintf(mesh_filename,"%s-%dK.sms",filename, M_numFaces(sim_mesh)/1000);
  else 
    sprintf(mesh_filename,"%s-%d.sms",filename, M_numFaces(sim_mesh));

  M_writeFMDB(sim_mesh, mesh_filename, useOriginal);
  std::cout << "\n[m3dc1_meshgen INFO] mesh is saved in Simmetrix-readable \""<<mesh_filename<<"\"\n";
  ...
  if (M_numFaces(sim_mesh)>1000)
    sprintf(mesh_filename,"%s-%dK.smb",filename, mesh->count(2)/1000);
  else
    sprintf(mesh_filename,"%s-%d.smb",filename, mesh->count(2));
  std::cout << "[m3dc1_meshgen INFO] mesh is saved in PUMI-readable \""<<mesh_filename<<"\"\n";
  mesh->writeNative(mesh_filename);

  if (M_numFaces(sim_mesh)>1000)
    sprintf(mesh_filename,"%s-%dK.vtk",filename, mesh->count(2)/1000);
  else
    sprintf(mesh_filename,"%s-%d.vtk",filename, mesh->count(2));
  std::cout << "[m3dc1_meshgen INFO] mesh is saved in Paraview \""<<mesh_filename<<"\"\n";
