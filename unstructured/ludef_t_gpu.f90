#define ADDTERM(xterm,prod,sscoef,ddcoef) \
    do i=1,prod%len; \
       nterm=nterm+1; \
       term(nterm)=xterm; \
       op1(nterm)=prod%op1(i); \
       op2(nterm)=prod%op2(i); \
       ssarray(:,nterm)=sscoef*prod%value79(:,i)*weight_79; \
       ddarray(:,nterm)=ddcoef*prod%value79(:,i)*weight_79; \
    enddo

!======================================================================
! Vorticity Equation
!======================================================================
subroutine vorticity_lin(nterm, term, op1, op2, ssarray, ddarray, advfield, &
     izone)
  ...
  integer, dimension(251), intent(out) :: term, op1, op2
  vectype, dimension(MAX_PTS,251), intent(out) :: ssarray, ddarray
  ...
  if(itime_independent.eq.0) then
     ADDTERM(u_g,temp,freq_fac,freq_fac*bdf)
  else
     ADDTERM(u_g,temp,freq_fac,0)
  endif
  if(numvar.ge.3) then
     ...
     temp = v1chin(rho79)
     if(itime_independent.eq.0) then
        ADDTERM(chi_g,temp,chiiner*freq_fac,chiiner*freq_fac*bdf)
     else
        ADDTERM(chi_g,temp,chiiner*freq_fac,0)
     endif
  end if
  ...
  temp = v1umu(vis79,vic79)
  ADDTERM(u_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  if(numvar.ge.2) then
     !tempx = v1vmu(trialx,lin,vis79,vic79)
     !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
     !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp*bdf)*dt*tempx
     temp = v1vmu(vis79,vic79)
     ADDTERM(vz_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  end if
  if(numvar.ge.3) then
     ...
     temp = v1chimu(vis79,vic79)
     ADDTERM(chi_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  end if
  ...
  if(linear.eq.0) then 
     ...
     temp = v1uun1(ph179,rho79) &
          + v1uun2(ph179,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)

     !tempx = v1uvn(trialx,lin,vz179,rho79)
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (.5-thimp*bdf)*dt*tempx
     temp = v1uvn1(vz179,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)

     !tempx = v1uchin(trialx,lin,ch179,rho79)       
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (.5-thimp*bdf)*dt*tempx
     temp = v1uchin1(ch179,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
     ...
     if(numvar.ge.2) then
        ...
        temp = v1vvn1(vz179,rho79) &
             + v1vvn2(vz179,rho79) &
             + v1uvn2(ph179,rho79)
        ADDTERM(vz_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)

        !tempx = v1vchin(trialx,lin,ch179,rho79)       
        !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
        !ddterm(:,vz_g) = ddterm(:,vz_g) + (.5-thimp*bdf)*dt*tempx
        temp = v1vchin1(ch179,rho79)
        ADDTERM(vz_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
     end if
     ...
     if(numvar.ge.3) then
        ...
        temp = v1uchin2(ph179,rho79) &
             + v1vchin2(vz179,rho79) &
             + v1chichin1(ch179,rho79) &
             + v1chichin2(ch179,rho79)
        ADDTERM(chi_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
     end if
  end if
  ...
  if(eqsubtract.eq.1) then
     ...
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (1.-thimp*bdf)*dt*tempx
     temp = v1uun1(ph079,rho79) &
          + v1uun2(ph079,rho79) &
          + v1uvn1(vz079,rho79) &
          + v1uchin1(ch079,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     ...
     if(numvar.ge.2) then
        ...
        !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
        !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp*bdf)*dt*tempx
        temp = v1uvn2(ph079,rho79) &
             + v1vvn1(vz079,rho79) &
             + v1vvn2(vz079,rho79) &
             + v1vchin1(ch079,rho79)
        ADDTERM(vz_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     end if
     ...
     if(numvar.ge.3) then
        ...
        !ssterm(:,chi_g) = ssterm(:,chi_g) -     thimp     *dt*tempx
        !ddterm(:,chi_g) = ddterm(:,chi_g) + (1.-thimp*bdf)*dt*tempx
        temp = v1uchin2(ph079,rho79) &
             + v1vchin2(vz079,rho79) &
             + v1chichin1(ch079,rho79) &
             + v1chichin2(ch079,rho79)
        ADDTERM(chi_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     end if
     ...
     if(idens.eq.1) then
        ...
        !temp = v1uun3    (ph079,ph079) &
             !+ v1uvn3    (ph079,vz079) &
             !+ v1vvn3    (vz079,vz079) &
             !+ v1uchin3  (ph079,ch079) &
             !+ v1vchin3  (vz079,ch079) &
             !+ v1chichin3(ch079,ch079)
        !ADDTERM(den_g,temp,0,dt)
     end if
  end if
  ...
  if(advfield.eq.1) then
     ...
     temp = v1psipsi1(pss79) &
          + v1psipsi2(pss79) &
          + v1psib1(bzs79)
     ADDTERM(psi_g,temp,0,dt)
     !if(numvar.ge.2) then
  ...
  ! Unsplit time-step
     ...
     if(linear.eq.0) then
        ...
        temp = v1psipsi1(ps179) &
             + v1psipsi2(ps179)
        ADDTERM(psi_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)

        !tempx = v1psib(trialx,lin,bz179)
        !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimp     *dt*tempx
        !ddterm(:,psi_g) = ddterm(:,psi_g) + (nv-thimp*bdf)*dt*tempx
        temp = v1psib1(bz179)
        ADDTERM(psi_g,temp,-thimp*dt,(nv-thimp*bdf)*dt)
        ...
        if(numvar.ge.2) then
           ...
           temp = v1psib2(ps179)
           ADDTERM(bz_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
        end if
     end if
     ...
     if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
        ...
        temp = v1psipsi1(ps079) &
             + v1psipsi2(ps079) &
             + v1psib1(bz079)
        ADDTERM(psi_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
        ...
        if(numvar.ge.2) then
           ...
           temp = v1psib2(ps079)
           ADDTERM(bz_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
        end if
     end if
  ...
  if(i3d.eq.1 .and. numvar.ge.2) then
     if(linear.eq.0) then
        ...
        temp = v1psif2(ps179) &
             + v1bf2  (bz179)
        ADDTERM(20,temp,-thimp_bf*dt,(.5-thimp_bf*bdf)*dt)
     end if
     if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
        ...
        temp = v1psif1(bfp079)
        ADDTERM(psi_g,temp,-thimp_bf*dt,(1-thimp_bf*bdf)*dt)

        !tempx = v1bf(trialx,lin,bfp079)
        !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimp_bf     *dt*tempx
        !ddterm(:,bz_g) = ddterm(:,bz_g) + (1.-thimp_bf*bdf)*dt*tempx
        temp = v1bf1(bfp079)
        ADDTERM(bz_g,temp,-thimp_bf*dt,(1-thimp_bf*bdf)*dt)
        ...
        temp = v1psif2(ps079) &
             + v1bf2  (bz079)
        ADDTERM(20,temp,-thimp_bf*dt,(1-thimp_bf*bdf)*dt)
     end if
     if(use_external_fields .and. linear.eq.0) then
...
!======================================================================
subroutine axial_vel_lin(nterm, term, op1, op2, ssarray, ddarray, advfield, &
     izone)
  ...
  temp = v2vn(rho79)
  if(itime_independent.eq.0) then
     ADDTERM(vz_g,temp,freq_fac,freq_fac*bdf)
  else
     ADDTERM(vz_g,temp,freq_fac,0)
  endif
  ...
  if(eqsubtract.eq.1) then
     ...
     !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
     !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp*bdf)*dt*tempx
     temp = v2vun1(ph079,rho79) &
          + v2vvn1(vz079,rho79) &
          + v2vvn2(vz079,rho79) &
          + v2vchin1(ch079,rho79)
     ADDTERM(vz_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     ...
     if(idens.eq.1) then
        ...
        !temp = v2vun3  (vz079,ph079) &
             !+ v2vvn3  (vz079,vz079) &
             !+ v2vchin3(vz079,ch079)
        !ADDTERM(den_g,temp,0,dt)
     end if
  end if
  ...
  ! Unsplit time-step
     ...
     if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
        ...
        !!ssterm(:,psi_g) = ssterm(:,psi_g) -     thimp     *dt*tempx
        !!ddterm(:,psi_g) = ddterm(:,psi_g) + (1.-thimp*bdf)*dt*tempx
        temp = v2psipsi1(ps079) &
             + v2psipsi2(ps079) &
             + v2psib1(bz079)
        ADDTERM(psi_g,temp,-thimp*dt,(1-thimp)*dt)

        !!tempx = v2psib(trialx,ps079,lin)
        !!ssterm(:,bz_g) = ssterm(:,bz_g) -     thimp     *dt*tempx
        !!ddterm(:,bz_g) = ddterm(:,bz_g) + (1.-thimp*bdf)*dt*tempx
        temp = v2psib2(ps079)
        ADDTERM(bz_g,temp,-thimp*dt,(1-thimp)*dt)
     end if
...
!======================================================================
subroutine compression_lin(nterm, term, op1, op2, ssarray, ddarray, advfield, &
     izone)
  ...
  !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
  !ddterm(:,u_g) = ddterm(:,u_g) + (1.-thimp*bdf)*dt*tempx
  temp = v3umu(vis79,vic79)
  ADDTERM(u_g,temp,-thimp*dt,(1-thimp*bdf)*dt)

  !tempx = v3vmu(trialx,lin,vis79,vic79)
  !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
  !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp*bdf)*dt*tempx
  temp = v3vmu(vis79,vic79)
  ADDTERM(vz_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  ...
  if(linear.eq.0) then 
     ...
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (.5-thimp*bdf)*dt*tempx
     temp = v3uun1(ph179,rho79) &
          + v3uun2(ph179,rho79) &
          + v3uvn1(vz179,rho79) &
          + v3uchin1(ch179,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
     ...
     !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
     !ddterm(:,vz_g) = ddterm(:,vz_g) + (.5-thimp*bdf)*dt*tempx
     temp = v3uvn2(ph179,rho79) &
          + v3vvn1(vz179,rho79) &
          + v3vvn2(vz179,rho79) &
          + v3vchin1(ch179,rho79)
     ADDTERM(vz_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
     ...
     !ssterm(:,chi_g) = ssterm(:,chi_g) -     thimp     *dt*tempx
     !ddterm(:,chi_g) = ddterm(:,chi_g) + (.5-thimp*bdf)*dt*tempx
     temp = v3uchin2(ph179,rho79) &
          + v3vchin2(vz179,rho79) &
          + v3chichin1(ch179,rho79) &
          + v3chichin2(ch179,rho79)
     ADDTERM(chi_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
  endif
  ...
  if(eqsubtract.eq.1) then             
     ...
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (1.-thimp*bdf)*dt*tempx
     temp = v3uun1(ph079,rho79) &
          + v3uun2(ph079,rho79) &
          + v3uvn1(vz079,rho79) &
          + v3uchin1(ch079,rho79)
     ADDTERM(u_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     ...
     !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp     *dt*tempx
     !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp*bdf)*dt*tempx
     temp = v3uvn2(ph079,rho79) &
          + v3vvn1(vz079,rho79) &
          + v3vvn2(vz079,rho79) &
          + v3vchin1(ch079,rho79)
     ADDTERM(vz_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     ...
     !ssterm(:,chi_g) = ssterm(:,chi_g) -     thimp     *dt*tempx
     !ddterm(:,chi_g) = ddterm(:,chi_g) + (1.-thimp*bdf)*dt*tempx
     temp = v3uchin2(ph079,rho79) &
          + v3vchin2(vz079,rho79) &
          + v3chichin1(ch079,rho79) &
          + v3chichin2(ch079,rho79)
     ADDTERM(chi_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
     ...
     if(idens.eq.1) then 
        ...
        !temp = v3uun3    (ph079,ph079) &
             !+ v3uvn3    (ph079,vz079) &
             !+ v3uchin3  (ph079,ch079) &
             !+ v3vvn3    (vz079,vz079) &
             !+ v3vchin3  (vz079,ch079) &
             !+ v3chichin3(ch079,ch079)
        !ADDTERM(den_g,temp,0,dt)
     end if
  end if
...
! Flux Equation
!======================================================================
subroutine flux_lin(nterm, term, op1, op2, ssarray, ddarray, izone)
  ...
  if(izone.ne.2) then
     ...
     temp = b1psieta1(eta79,vzt79,eta_mod.eq.1) &
          + b1psieta2(eta79,vzt79,eta_mod.eq.1)
  else
     !tempx = b1psieta1(trialx,lin,eta79  ,vzt79,eta_mod.eq.1) &
     !     + b1psieta2(trialx,lin,etaRZ79,vzt79,eta_mod.eq.1)
     temp = b1psieta1(eta79,vzt79,eta_mod.eq.1) &
          + b1psieta2(etaRZ79,vzt79,eta_mod.eq.1)
  endif
  !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimp     *dt*tempx
  !ddterm(:,psi_g) = ddterm(:,psi_g) + (1.-thimp*bdf)*dt*tempx
  ADDTERM(psi_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  ...
  !ssterm(:,psi_g) = ssterm(:,psi_g) + tempx
  !if(itime_independent.eq.0) ddterm(:,psi_g) = ddterm(:,psi_g) + tempx*bdf
  temp = b1psi() + (-1.)*b1psid(ni79)
  if(itime_independent.eq.0) then
     ADDTERM(psi_g,temp,freq_fac,freq_fac*bdf)
  else
     ADDTERM(psi_g,temp,freq_fac,0)
  endif
  ...
  if(linear.eq.0) then 
     ...
     temp = b1psiu2  (ps179)
     ADDTERM(u_g,temp,-thimpb*dt,(.5-thimpb*bdf)*dt)

     !tempx = b1bu    (trialx,bz179,lin)
     !ssterm(:,u_g) = ssterm(:,u_g) -     thimp     *dt*tempx
     !ddterm(:,u_g) = ddterm(:,u_g) + (nv-thimp*bdf)*dt*tempx
     temp = b1bu2  (bz179)
     ADDTERM(u_g,temp,-thimpb*dt,(nv-thimpb*bdf)*dt)
     ...
     if(numvar.ge.3) then
        ...
        temp = b1psichi2(ps179) &
             + b1bchi2  (bz179)
        ADDTERM(chi_g,temp,-thimpb*dt,(.5-thimpb*bdf)*dt)
     endif
  end if
  ...
  if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
     ...
     if(numvar.ge.3) then
        ...
        temp = b1psichi2  (ps079) &
             + b1bchi2    (bz079)
        ADDTERM(chi_g,temp,-thimpb*dt,(1-thimpb*bdf)*dt)
     endif
  end if
  ...
  if(i3d.eq.1 .and. numvar.ge.2) then
     if(linear.eq.0) then
        ...
        if(numvar.ge.3) then
           ...
           temp = b1fchi2(bfp179)
           ADDTERM(chi_g,temp,-thimp_bf*dt,(.5-thimp_bf*bdf)*dt)
        end if
     ...
     if(eqsubtract.eq.1) then
        ...
        temp = b1fu2(bfp079)
        ADDTERM(u_g,temp,-thimp_bf*dt,(1.-thimp_bf*bdf)*dt)
  
        !tempx = b1fv(trialx,bfp079,lin)
        !ssterm(:,vz_g) = ssterm(:,vz_g) -     thimp_bf     *dt*tempx
        !ddterm(:,vz_g) = ddterm(:,vz_g) + (1.-thimp_bf*bdf)*dt*tempx
        temp = b1fv2(bfp079)
        ADDTERM(vz_g,temp,-thimp_bf*dt,(1.-thimp_bf*bdf)*dt)
        ...
        if(numvar.ge.3) then
           ...
           temp = b1fchi2(bfp079)
           ADDTERM(chi_g,temp,-thimp_bf*dt,(1.-thimp_bf*bdf)*dt)
        end if
...
!======================================================================
subroutine axial_field_lin(nterm, term, op1, op2, ssarray, ddarray, &
     izone)
  ...
  if(izone.ne.2) then
    ...
    temp = b2psieta1(eta79)
  else
    !tempx = b2psieta(trialx,lin,etaRZ79)
    temp = b2psieta1(etaRZ79)
  endif
  !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimp     *dt*tempx
  !ddterm(:,psi_g) = ddterm(:,psi_g) + (1.-thimp*bdf)*dt*tempx
  ADDTERM(psi_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  if(izone.ne.2) then
    ...
    temp = b2beta1(eta79,vzt79)
  else
    !tempx = b2beta(trialx,lin,etaRZ79,vzt79)
    temp = b2beta1(etaRZ79,vzt79)
  endif
  !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimp     *dt*tempx
  !ddterm(:,bz_g) = ddterm(:,bz_g) + (1.-thimp*bdf)*dt*tempx
  ADDTERM(bz_g,temp,-thimp*dt,(1-thimp*bdf)*dt)
  ...
  temp = b2b() + (-1.)*b2bd(ni79)
  if(itime_independent.eq.0) then
     ADDTERM(bz_g,temp,freq_fac,freq_fac*bdf)
  else
     ADDTERM(bz_g,temp,freq_fac,0)
  endif
...
! Pressure Equation
!======================================================================
subroutine pressure_lin(nterm, term, op1, op2, ssarray, ddarray, &
  ...
  if(linear.eq.1) then
     ...
     if(idens.eq.1) then
        ...
        !ssterm(:,den_g) = ssterm(:,den_g) -       thimp     *dt*tempx
        !ddterm(:,den_g) = ddterm(:,den_g) + (1. - thimp*bdf)*dt*tempx
        temp = kappat_lin_pn2(pp079)
        ADDTERM(den_g,temp,-thimpb*dt,(1.-thimpb*bdf)*dt)
     end if
  else 
  ...
  if(linear.eq.0) then
     ...
     !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimpf     *dt*tempx
     !ddterm(:,psi_g) = ddterm(:,psi_g) + (.5-thimpf*bdf)*dt*tempx
     temp = b3psipsieta1(ps179,eta79,nre179) &
          + b3psipsieta1(ps179,eta79,nre179)
     ADDTERM(psi_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)
     ...
     if(numvar.ge.2) then
        ...
        !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimpf     *dt*tempx
        !ddterm(:,bz_g) = ddterm(:,bz_g) + (.5-thimpf*bdf)*dt*tempx
        temp = b3bbeta1(bz179,eta79) &
             + b3bbeta1(bz179,eta79)
        ADDTERM(bz_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)

        !tempx = b3psibeta(trialx,lin,bz179,eta79,nre179) 
        !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimpf     *dt*tempx
        !ddterm(:,psi_g) = ddterm(:,psi_g) + (.5-thimpf*bdf)*dt*tempx
        temp = b3psibeta1(bz179,eta79,nre179)
        ADDTERM(psi_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)

        !tempx = b3psibeta(trialx,ps179,lin,eta79,nre179)
        !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimpf     *dt*tempx
        !ddterm(:,bz_g) = ddterm(:,bz_g) + (.5-thimpf*bdf)*dt*tempx
        temp = b3psibeta2(ps179,eta79,nre179)
        ADDTERM(bz_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)
        if(i3d .eq. 1) then
           !tempx = b3psifeta(trialx,lin,bfp179,eta79,nre179)
           !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimpf     *dt*tempx
           !ddterm(:,psi_g) = ddterm(:,psi_g) + (.5-thimpf*bdf)*dt*tempx
           temp = b3psifeta1(bfp179,eta79,nre179)
           ADDTERM(psi_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)

           !tempx = b3psifeta(trialx,ps179,lin,eta79,nre179)
           !r_bf = r_bf -     thimp_bf     *dt*tempx
           !q_bf = q_bf + (.5-thimp_bf*bdf)*dt*tempx
           temp = b3psifeta2(ps179,eta79,nre179)
           ADDTERM(20,temp,-thimp_bf*dt,(.5-thimp_bf*bdf)*dt)

           !tempx = b3bfeta(trialx,lin,bfp179,eta79,nre179)
           !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimpf     *dt*tempx
           !ddterm(:,bz_g) = ddterm(:,bz_g) + (.5-thimpf*bdf)*dt*tempx
           temp = b3bfeta1(bfp179,eta79,nre179)
           ADDTERM(bz_g,temp,-thimpf*dt,(.5-thimpf*bdf)*dt)

           !tempx = b3bfeta(trialx,bz179,lin,eta79,nre179)
           !r_bf = r_bf -     thimp_bf     *dt*tempx
           !q_bf = q_bf + (.5-thimp_bf*bdf)*dt*tempx
           temp = b3bfeta2(bz179,eta79,nre179)
           ADDTERM(20,temp,-thimp_bf*dt,(.5-thimp_bf*bdf)*dt)
           ...
           !r_bf = r_bf -     thimp_bf     *dt*tempx
           !q_bf = q_bf + (.5-thimp_bf*bdf)*dt*tempx
           temp = b3ffeta1(bfp179,eta79,nre179) &
                + b3ffeta1(bfp179,eta79,nre179)
           ADDTERM(20,temp,-thimp_bf*dt,(.5-thimp_bf*bdf)*dt)
        endif
  ...
  if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
     ...
     !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimpf     *dt*tempx
     !ddterm(:,psi_g) = ddterm(:,psi_g) + (1.-thimpf*bdf)*dt*tempx
     temp = b3psipsieta1(ps079,eta79,nre079) &
          + b3psipsieta1(ps079,eta79,nre079)
     ADDTERM(psi_g,temp,-thimpf*dt,(1.-thimpf*bdf)*dt)
     ...
     if(numvar.ge.2) then
        ...
        !ssterm(:,bz_g) = ssterm(:,bz_g) -     thimpf     *dt*tempx
        !ddterm(:,bz_g) = ddterm(:,bz_g) + (1.-thimpf*bdf)*dt*tempx
        temp = b3bbeta1(bz079,eta79) &
             + b3bbeta1(bz079,eta79)
        ADDTERM(bz_g,temp,-thimpf*dt,(1.-thimpf*bdf)*dt)

        !tempx = b3psibeta(trialx,lin,bz079,eta79,nre079)
        !ssterm(:,psi_g) = ssterm(:,psi_g) -     thimpf     *dt*tempx
        !ddterm(:,psi_g) = ddterm(:,psi_g) + (1.-thimpf*bdf)*dt*tempx
        temp = b3psibeta1(bz079,eta79,nre079)
        ADDTERM(psi_g,temp,-thimpf*dt,(1.-thimpf*bdf)*dt)
        ...
        if(i3d .eq. 1) then
           ...
           !r_bf = r_bf -     thimp_bf     *dt*tempx
           !q_bf = q_bf + (1.-thimp_bf*bdf)*dt*tempx
           temp = b3bfeta2(bz079,eta79,nre079)
           ADDTERM(20,temp,-thimp_bf*dt,(1.-thimp_bf*bdf)*dt)
        endif
     end if
  endif
  ...
  if(linear.eq.0) then
     ...
     temp = p1pu1  (ph179) &
          + p1pv1  (vz179) &
          + p1pchi1(ch179)
     ADDTERM(pp_g,temp,-thimpb*dt,(.5-thimpb*bdf)*dt)
  endif
...
!#endif
  ...
  if(kappar.ne.0.) then
     ...
     if(linear.eq.1) then
        ...
        !ssterm(:,psi_g) = ssterm(:,psi_g) -       thimpf     *dt*tempx
        !ddterm(:,psi_g) = ddterm(:,psi_g) + (1. - thimpf*bdf)*dt*tempx
        temp = p1psipsipnkappar1(ps079,pp079,n079,ieq_bdotgradt) &
             + p1psipsipnkappar2(ps079,pp079,n079,1) &
             + p1psibpnkappar1  (bz079,pp079,n079,ieq_bdotgradt,1)
        ADDTERM(psi_g,temp,-thimpf*dt,(1.-thimpf*bdf)*dt)
        ...
        if(numvar.ge.2) then
           ...
           !ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
           !ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
           temp = p1psibpnkappar2(ps079,pp079,n079,1,ieq_bdotgradt) &
                + p1bbpnkappar1  (bz079,pp079,n079,ieq_bdotgradt) &
                + p1bbpnkappar2  (bz079,pp079,n079,1)
           ADDTERM(bz_g,temp,-thimpf*dt,(1.-thimpf*bdf)*dt)
        end if
        ...
        !ssterm(:,pp_g) = ssterm(:,pp_g) -       thimp     *dt*tempx
        !ddterm(:,pp_g) = ddterm(:,pp_g) + (1. - thimp*bdf)*dt*tempx
        temp = p1psipsipnkappar3 (ps079,ps079,n079,1) &
             + p1psibpnkappar3(ps079,bz079,n079,1,1) &
             + p1bbpnkappar3  (bz079,bz079,n079,1)
        ADDTERM(pp_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
        ...
        if(idens.eq.1) then
           ...
           temp = p1psipsipnkappar4 (ps079,ps079,p079,1) &
                + p1psibpnkappar4(ps079,bz079,p079,1,1) &
                + p1bbpnkappar4  (bz079,bz079,p079,1)
           ADDTERM(den_g,temp,thimpf*dt,-(1.-thimpf*bdf)*dt)
        end if
        ...
        if(i3d.eq.1 .and. numvar.ge.2) then
           ...
           !r_bf = r_bf -       thimp_bf     *dt*tempx
           !q_bf = q_bf + (1. - thimp_bf*bdf)*dt*tempx
           temp = p1psifpnkappar2 (ps079,pp079,n079,1,ieq_bdotgradt) &
                + p1bfpnkappar2   (bz079,pp079,n079,1,ieq_bdotgradt)
           ADDTERM(20,temp,-thimp_bf*dt,(1.-thimp_bf*bdf)*dt)
        endif
...
!======================================================================
subroutine temperature_lin(nterm, term, op1, op2, ssarray, ddarray, &
...
! NOTE:  iadiabat=1 is correct form;   adiabat=0 is for backwards compatibility (6/2/16)
  ...
  if(no_vdg_T .eq. 0) then   ! debug
    if(linear.eq.0) then
       ...
       !ssterm(:,pp_g) = ssterm(:,pp_g) -     thimp     *dt*tempx
       !ddterm(:,pp_g) = ddterm(:,pp_g) + (.5-thimp*bdf)*dt*tempx
       temp = t3tnu1(nnw79,ph179) &
            + t3tnv1(nnw79,vz179) &
            + t3tnchi1(nnw79,ch179)
       ADDTERM(pp_g,temp,-thimp*dt,(.5-thimp*bdf)*dt)
    endif
    if(eqsubtract.eq.1) then
       !tempx = t3tnu(trialx,pp079,nnw79,lin)
       !ssterm(:,u_g) = ssterm(:,u_g) -     thimpb     *dt*tempx
       !ddterm(:,u_g) = ddterm(:,u_g) + (1.-thimpb*bdf)*dt*tempx
       temp = t3tnu3(pp079,nnw79)
       ADDTERM(u_g,temp,-thimpb*dt,(1.-thimpb*bdf)*dt)
  ...
  !ssterm(:,pp_g) = ssterm(:,pp_g) -     thimp     *dt*tempx
  !ddterm(:,pp_g) = ddterm(:,pp_g) + (1.-thimp*bdf)*dt*tempx
  temp = b3tekappa1(kap79,vzt79)
  if((ipres.eq.0 .and. numvar.ge.3) .or. (ipres.eq.1 .and. numvar.lt.3)) then
     ! Add ion heat flux
     tempx = (1. + kappai_fac*(1.-pefac)/pefac)*tempx
  else
     if(.not. electron_temperature) then 
        temp = kappai_fac*temp
     end if
  end if
  ADDTERM(pp_g,temp,-thimp*dt,(1.-thimp*bdf)*dt)
  ...
  !if(db.ne.0.) then
     ...
     !if(i3d.eq.1 .and. numvar.ge.2) then
        !if(linear.eq.0) then
           !if(numvar.ge.3) then
              !tempx = b3pefd(trialx,lin,bf179,ni79)*db
              !ssterm(:,pe_g) = ssterm(:,pe_g) -     thimp_bf     *dt*tempx
              !ddterm(:,pe_g) = ddterm(:,pe_g) + (.5-thimp_bf*bdf)*dt*tempx
           !end if
        ...
        !if(eqsubtract.eq.1) then
           !if(numvar.ge.3) then
              !tempx = b3pefd(trialx,lin,bf079,ni79)*db
              !ssterm(:,pe_g) = ssterm(:,pe_g) -     thimp_bf     *dt*tempx
              !ddterm(:,pe_g) = ddterm(:,pe_g) + (1.-thimp_bf*bdf)*dt*tempx
           !end if
...
!
          ...
          ! ssterm(:,pp_g) = ssterm(:,pp_g) -          thimp     *dt*tempx
          ! ddterm(:,pp_g) = ddterm(:,pp_g) + (1.    - thimp*bdf)*dt*tempx
          temp = tepsipsikappar3(pstx79,pstx79,b2i79,kar79) &
               + tepsibkappar3  (pstx79,bztx79,b2i79,kar79) &
               + tebbkappar3    (bztx79,bztx79,b2i79,kar79)
          ADDTERM(pp_g,temp,-thimp*dt*fac,(1.-thimp*bdf)*dt*fac)
          if(i3d.eq.1 .and. numvar.ge.2) then
             ! tempx = tepsifkappar(trialx,pstx79,bftx79,lin,b2i79,kar79) &
             !      + tebfkappar  (trialx,bztx79,bftx79,lin,b2i79,kar79) &
             !      + teffkappar  (trialx,bftx79,bftx79,lin,b2i79,kar79)
             ! ssterm(:,pp_g) = ssterm(:,pp_g) -          thimp     *dt*tempx
             ! ddterm(:,pp_g) = ddterm(:,pp_g) + (1.    - thimp*bdf)*dt*tempx
             temp = tepsifkappar3(pstx79,bfptx79,b2i79,kar79) &
                  + tebfkappar3  (bztx79,bfptx79,b2i79,kar79) &
                  + teffkappar3  (bfptx79,bfptx79,b2i79,kar79)
             ADDTERM(pp_g,temp,-thimp*dt*fac,(1.-thimp*bdf)*dt*fac)
          endif
       ...
       if(eqsubtract.eq.1 .or. icsubtract.eq.1) then
          ...
          ! ssterm(:,psi_g) = ssterm(:,psi_g) -          thimpf     *dt*tempx*.5
          ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. -    thimpf*bdf)*dt*tempx*.5
          temp = tepsipsikappar1(ps179,pp079,b2i79,kar79) &
               + tepsipsikappar2(ps179,pp079,b2i79,kar79) &
               + tepsibkappar1  (bz179,pp079,b2i79,kar79)
          ADDTERM(psi_g,temp,-thimpf*dt*0.5*fac,(1.-thimpf*bdf)*dt*0.5*fac)
          ...
          ! ssterm(:,bz_g) = ssterm(:,bz_g) -          thimpf     *dt*tempx*.5
          ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. -    thimpf*bdf)*dt*tempx*.5
          temp = tepsibkappar2(ps179,pp079,b2i79,kar79) &
               + tebbkappar1  (bz179,pp079,b2i79,kar79) &
               + tebbkappar2  (bz179,pp079,b2i79,kar79)
          ADDTERM(bz_g,temp,-thimpf*dt*0.5*fac,(1.-thimpf*bdf)*dt*0.5*fac)
          ...
          ! ssterm(:,psi_g) = ssterm(:,psi_g) -       thimpf     *dt*tempx
          ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsipsikappar1(ps079,pp079,b2i79,kar79) &
               + tepsipsikappar2(ps079,pp079,b2i79,kar79) &
               + tepsibkappar1  (bz079,pp079,b2i79,kar79)
          ADDTERM(psi_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
          ...
          ! ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
          ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsibkappar2(ps079,pp079,b2i79,kar79) &
               + tebbkappar1  (bz079,pp079,b2i79,kar79) &
               + tebbkappar2  (bz079,pp079,b2i79,kar79)
          ADDTERM(bz_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
 
          if(i3d.eq.1 .and. numvar.ge.2) then
             ! tempx = tepsifkappar(trialx,lin,bf179,pp079,b2i79,kar79)
             ! ssterm(:,psi_g) = ssterm(:,psi_g) -          thimpf     *dt*tempx*.5
             ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. -    thimpf*bdf)*dt*tempx*.5
             temp = tepsifkappar1(bf179,pp079,b2i79,kar79)
             ADDTERM(psi_g,temp,-thimpf*dt*0.5*fac,(1.-thimpf*bdf)*dt*0.5*fac)

             ! tempx = tebfkappar(trialx,lin,bf179,pp079,b2i79,kar79)
             ! ssterm(:,bz_g) = ssterm(:,bz_g) -          thimpf     *dt*tempx*.5
             ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. -    thimpf*bdf)*dt*tempx*.5
             temp = tebfkappar1(bf179,pp079,b2i79,kar79)
             ADDTERM(bz_g,temp,-thimpf*dt*0.5*fac,(1.-thimpf*bdf)*dt*0.5*fac)

             ! tempx = teffkappar(trialx,lin,bf179,pp079,b2i79,kar79) &
             !      + teffkappar(trialx,bf179,lin,pp079,b2i79,kar79) &
             !    + tepsifkappar(trialx,ps179,lin,pp079,b2i79,kar79) &
             !      + tebfkappar(trialx,bz179,lin,pp079,b2i79,kar79)
             ! r_bf = r_bf -          thimp_bf     *dt*tempx*.5
             ! q_bf = q_bf + (1. -    thimp_bf*bdf)*dt*tempx*.5
             temp = teffkappar1 (bf179,pp079,b2i79,kar79) &
                  + teffkappar2 (bf179,pp079,b2i79,kar79) &
                  + tepsifkappar2 (ps179,pp079,b2i79,kar79) &
                  + tebfkappar2  (bz179,pp079,b2i79,kar79)
             ADDTERM(20,temp,-thimp_bf*dt*0.5*fac,(1.-thimp_bf*bdf)*dt*0.5*fac)
  
             ! tempx = tepsifkappar(trialx,lin,bf079,pp079,b2i79,kar79)
             ! ssterm(:,psi_g) = ssterm(:,psi_g) -       thimpf     *dt*tempx
             ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. - thimpf*bdf)*dt*tempx
             temp = tepsifkappar1(bf079,pp079,b2i79,kar79)
             ADDTERM(psi_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
             
             ! tempx = tebfkappar(trialx,lin,bf079,pp079,b2i79,kar79)
             ! ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
             ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
             temp = tebfkappar1(bf079,pp079,b2i79,kar79)
             ADDTERM(bz_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)

             ! tempx = teffkappar(trialx,lin,bf079,pp079,b2i79,kar79) &
             !      + teffkappar(trialx,bf079,lin,pp079,b2i79,kar79) &
             !    + tepsifkappar(trialx,ps079,lin,pp079,b2i79,kar79) &
             !      + tebfkappar(trialx,bz079,lin,pp079,b2i79,kar79)
             ! r_bf = r_bf -       thimp_bf     *dt*tempx
             ! q_bf = q_bf + (1. - thimp_bf*bdf)*dt*tempx
             temp = teffkappar1 (bf079,pp079,b2i79,kar79) &
                  + teffkappar2 (bf079,pp079,b2i79,kar79) &
                  + tepsifkappar2 (ps079,pp079,b2i79,kar79) &
                  + tebfkappar2  (bz079,pp079,b2i79,kar79)
             ADDTERM(20,temp,-thimp_bf*dt*fac,(1.-thimp_bf*bdf)*dt*fac)
          end if
       endif
       ...
       if(use_external_fields) then
          ...
          ! ssterm(:,psi_g) = ssterm(:,psi_g) -       thimpf     *dt*tempx
          ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsipsikappar1(psx79,pp079,b2i79,kar79) &
               + tepsipsikappar2(psx79,pp079,b2i79,kar79) &
               + tepsibkappar1  (bzx79,pp079,b2i79,kar79)
          ADDTERM(psi_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
          ...
          ! ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
          ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsibkappar2(psx79,pp079,b2i79,kar79) &
               + tebbkappar1  (bzx79,pp079,b2i79,kar79) &
               + tebbkappar2  (bzx79,pp079,b2i79,kar79)
          ADDTERM(bz_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)

          ! tempx = tepsifkappar(trialx,lin,bfx79,pp079,b2i79,kar79)
          ! ssterm(:,psi_g) = ssterm(:,psi_g) -       thimpf     *dt*tempx
          ! ddterm(:,psi_g) = ddterm(:,psi_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsifkappar1(bfx79,pp079,b2i79,kar79)
          ADDTERM(psi_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
                 
          ! tempx = tebfkappar(trialx,lin,bfx79,pp079,b2i79,kar79)
          ! ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
          ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tebfkappar1(bfx79,pp079,b2i79,kar79)
          ADDTERM(bz_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)

          ! tempx = teffkappar(trialx,lin,bfx79,pp079,b2i79,kar79) &
          !      + teffkappar(trialx,bfx79,lin,pp079,b2i79,kar79)
          ! r_bf = r_bf -       thimp_bf     *dt*tempx
          ! q_bf = q_bf + (1. - thimp_bf*bdf)*dt*tempx
          temp = teffkappar1(bfx79,pp079,b2i79,kar79) &
               + teffkappar2(bfx79,pp079,b2i79,kar79)
          ADDTERM(20,temp,-thimp_bf*dt*fac,(1.-thimp_bf*bdf)*dt*fac)
       end if
       ...
       ! Assumes no contribution from equilibrium f
        ...
        temp = tepsipsikappar1(ps079,pp079,b2i79,kar79) &
             + tepsipsikappar2(ps079,pp079,b2i79,kar79) &
             + tepsibkapparl1 (bz079,pp079,b2i79,kar79)
        ADDTERM(psi_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
       ...
       if(numvar.ge.2) then
          ...
          ! ssterm(:,bz_g) = ssterm(:,bz_g) -       thimpf     *dt*tempx
          ! ddterm(:,bz_g) = ddterm(:,bz_g) + (1. - thimpf*bdf)*dt*tempx
          temp = tepsibkapparl2(ps079,pp079,b2i79,kar79) &
               + tebbkapparl1  (bz079,pp079,b2i79,kar79) &
               + tebbkapparl2  (bz079,pp079,b2i79,kar79)
          ADDTERM(bz_g,temp,-thimpf*dt*fac,(1.-thimpf*bdf)*dt*fac)
        end if
        ...
        ! ssterm(:,pp_g) = ssterm(:,pp_g) -       thimp     *dt*tempx
        ! ddterm(:,pp_g) = ddterm(:,pp_g) + (1. - thimp*bdf)*dt*tempx
        !temp = tepsipsikappar3 (ps079,ps079,b2i79,kar79) &
        temp = tepsipsikappar3 (ps079,ps079,b2i79,kar79) &
             + tepsibkapparl3  (ps079,bz079,b2i79,kar79) &
             + tebbkapparl3    (bz079,bz079,b2i79,kar79)
        ADDTERM(pp_g,temp,-thimp*dt*fac,(1.-thimp*bdf)*dt*fac)
     !
       ...
       if(i3d.eq.1 .and. numvar.ge.2) then
          ...
          ! r_bf = r_bf -       thimp_bf     *dt*tempx
          ! q_bf = q_bf + (1. - thimp_bf*bdf)*dt*tempx
          temp = tepsifkappar2 (ps079,pp079,b2i79,kar79) &
               + tebfkapparl2  (bz079,pp079,b2i79,kar79)
          ADDTERM(20,temp,-thimp_bf*dt*fac,(1.-thimp_bf*bdf)*dt*fac)
       endif
...
subroutine ludefvel_n(itri)
  ...
  integer, dimension(251) :: term, op1, op2
  vectype, dimension(MAX_PTS,251) :: ssarray, ddarray
  vectype, dimension(dofs_per_element,dofs_per_element,251) :: ssterm, ddterm
  integer :: nterm, iterm, ipoint
  vectype, dimension(MAX_PTS) :: tempss, tempdd
  ...
  do k=1,numvar
     ...
     case(1)
        ...
        !end do
        call vorticity_lin(nterm, term, op1, op2, ssarray, ddarray, advfield,izone)
     case(2)
        !do j=1,dofs_per_element
           !call axial_vel_lin(mu79,nu79(j,:,:), &
                !ss(:,j,:),dd(:,j,:),r_bf(:,j),q_bf(:,j),advfield,izone)
        !end do
        call axial_vel_lin(nterm, term, op1, op2, ssarray, ddarray, advfield,izone)
     case(3)
        !do j=1,dofs_per_element
        ...
        do i=1,dofs_per_element
           do j=1,dofs_per_element
              !$acc loop seq
              do iterm=1,nterm
                 if (term(iterm)==20) then
                    r_bf(i,j)=r_bf(i,j)+ssterm(i,j,iterm)
                    q_bf(i,j)=q_bf(i,j)+ddterm(i,j,iterm)
                 else
...
!!$     if(ipressplit.ne.0 .and. numvar.ge.3) maxk = maxk - 1   !  is this generally valid?
  ...
  do k=1,maxk
     ...
     if (ieq(k).eq.psi_i) then
        if(.not.surface_int) then
           call flux_lin(nterm, term, op1, op2, ssarray, ddarray, izone)
        endif
     else if(ieq(k).eq.bz_i .and. numvar.ge.2) then
        call axial_field_lin(nterm,term, op1, op2, ssarray, ddarray, &
             izone)
...
subroutine ludefpres_n(itri)
  ...
  integer :: nterm, iterm, ipoint
  vectype, dimension(dofs_per_element,dofs_per_element,251) :: ssterm, ddterm
  !vectype, dimension(MAX_PTS,dofs_per_element,30) :: tempss, tempdd
  vectype, dimension(MAX_PTS) :: tempss, tempdd
  real :: tstart, tend
     ...
     if(ipressplit.eq.0) then
        ...
        case(1)
           call pressure_lin(nterm, term, op1, op2, ssarray, ddarray, &
                .true., thimpf, izone)
        case(2)
           call temperature_lin(nterm, term, op1, op2, ssarray, ddarray, &
                .true., thimpf, izone)
        end select
     endif ! on ipressplit
