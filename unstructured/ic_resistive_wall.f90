...
contains
  ...
  subroutine analytic_response_matrix
    ...
    m = mpol
    do m=-30, 30
       fac = (1./(nodes*a))*bessk(m,ka)/besskp(m,ka)
       fac1 = -m/ka * fac
       fac2 = (0.,1.) * fac
       do i=1,nodes+1
          do j=1,nodes+1
             zgrbth(i,j) = zgrbth(i,j) + fac1*sin(m*(theta(i) - theta(j)))
             zgrbph(i,j) = zgrbph(i,j) + fac2*cos(m*(theta(i) - theta(j)))
          enddo
       enddo
    end do
...
subroutine resistive_wall_test_per(x, z)
  ...
  real, intent(in) :: x, z
  real :: theta, r, co, sn, ram, Im_kr, Imp_kr, Impp_kr
  real :: bessi
  complex :: fac
