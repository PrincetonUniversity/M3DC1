...
\begin{eqnarray}
  ...
  + 
  \begin{pmatrix}
    \cola{R^v_{1 1}} & \colb{R^v_{1 2}} & R^v_{1 3}\\
    \colb{R^v_{2 1}} & \colb{R^v_{2 2}} & R^v_{2 3}\\
          R^v_{3 1}  &       R^v_{3 2}  & R^v_{3 3}\\
  \end{pmatrix} 
  \begin{pmatrix}
    \cola{\psi}\\ \colb{I}\\ p
  \end{pmatrix}^{(n)} \nonumber
...
\begin{tabular}{lll}
  ...
  \texttt{com\_bc}& 1 & 1: $\nabla^2 \chi = 0$\\
  \texttt{p\_pc}  & 0 & 0: $\partial_t p = 0$; 1: $\n \cdot \grad{p} =
    0$\\
...
\begin{tabular}{llll}
  ...
  \texttt{thimp\_ohm} & \texttt{thimp} & CN impliciness of ohmic heating terms
\end{tabular}
...
\begin{tabular}{llll}
  ...
  \texttt{th\_gs} & 0.5 & Relaxation paramter for iteration
\end{tabular}
