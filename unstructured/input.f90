...
subroutine set_defaults
  ...
  call add_var_int("iHT", iHT, 0, &
       "1: Include hot-tail runaway electron source", model_grp)
  call add_var_int("iHT_slice", iHT_slice, -1, &
       "Time slice from which to load initial temperature and density for the hot-tail model", model_grp)
  ...
  call add_var_int("iremc_geom", iremc_geom, 0, & ! RiD
       "1: model REMC coil as discretized 3D Biot-Savart arcs; 2: same discretized segment shape but each segment modeled as an axisymmetric coil() loop; either replaces the axisymmetric two-sector model (irmp=3 only)", model_grp)
  call add_var_int("remc_nseg", remc_nseg, 2, "Number of REMC coil toroidal arc segments (iremc_geom=1,2)", model_grp)
  call add_var_double("remc_Rpos", remc_Rpos, 0., "REMC coil major radius [m] (iremc_geom=1,2)", model_grp)
