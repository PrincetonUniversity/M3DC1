module pellet
  ...
  real :: n_g, subl, T_S, Mach, f_l, r_p
  real :: C_abl, Xp_abl, Xn_abl, a_Te, b_Te, c_Te, d_Te
...
!    use diagnostics
  ...
  subroutine calculate_parks_model
    ...
    case(1)
       ...
       pellet_rate = 4.*PI*(l0_norm*pellet_var)**2*q_s*shield_p*f_b*0.906!/(1.e-3*n_g*(subl+10./3.*T_s))
    ...
    case(2)
       ...
       Xn_abl = 8.1468e-9*(5./3.-1.)**(1./3.)*f_l**(1./3.)*6.941**(-1./3.)*(r_p*1.e2)**(4./3.)&
            *(n0_norm*nsource_pel)**(1./3.)*(te_norm*temp_pel)**(11./6.)*B_Li**(2./3.)

       Xp_abl = (8.1468e-9/(4.*4.*atan(1.)*0.534))*(5./3.-1.)**(1./3.)*f_l**(1./3.)*6.941**(2./3.)*(r_p*1.e2)**(-2./3.)&
            *(n0_norm*nsource_pel)**(1./3.)*(te_norm*temp_pel)**(11./6.)*B_Li**(2./3.)
       ...
       C_abl = a_Te*log(1.+b_Te*(r_p*1.e2)**(2./3.)*(nsource_pel/0.45)**(2./3.))/&
            log(c_Te+d_Te*(r_p*1.e2)**(2./3.)*(nsource_pel/0.45)**(2./3.))

       pellet_rate = 6.022e23*C_abl*Xn_abl*dt*t0_norm/(1.e6*n0_norm)

       r_p = r_p-dt*t0_norm*C_abl*Xp_abl*1.e-2
