...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine equi(size,r,p,dp,q,dq,Bz,Bp,k,i_system,q0,q_edge,beta,i_solver,r0,rs,m,n,is)
      implicit none
      integer size
      ...
      real, dimension(size):: r,p,q,dp,dq,integral1,integral2,Bz,Bp,dBz,jz,dBp,fx,integral1bis,Bzbis,integral2bis,Ir,dq_an
...
c Calculation of max beta to have q0 < 1
            call mywrite( ( (real(m)/real(n)/k*(1./r0)/(1+(rs/r0)**2))**2 + 2 * Ir(is) - (2. * Ir(1) + 1./r0**2/k**2) ) / ((real(m)/real(n)/k*(1./r0)/(1+(rs/r0)**2))**2 + 2 * Ir(is) - (1.-rs**2)*(2. * Ir(1) + 1./r0**2/k**2)) ,'beta max for q0<1 : ')
...
c$$$            j0 = Bz_edge* 4. * k / 2.8
            j0=1./sqrt((real(m)/real(n)/k*(0.5-rs**2/4.))**2+2.*(rs**2/4. - 3./16.* rs**4 + rs**6/24. - 5./48.))
            write(*,*) 'j0 =',j0
...
ccccccccccccccccccccccccccccccccccccccccccccccccc
cccc   (FTZ Reversed Shearing ) ccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccccc
         if (i_solver==3) then
            is_array=minloc(abs(r-rs))
            is=is_array(1)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccsubroutine main_tok_r045 ccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      subroutine main_tok_r045(size,size_plot,size_plot2) 
      use innerc_module
      implicit none
      ...
      real epsd,rmatch
      real E,F,G,H,K0
      real, dimension(size):: p,q,dp,dq,Bz,Bp,Bz2,Bp2,alpha_DL_array
...
c     real, dimension(10)::iend_array
      ...
      integer maxalphaloc
      integer i0p
      complex, dimension(1610,3)::yo,ye
      real co, ce
      real, dimension(1610)::xary
...
c     i_solver=1 : Bp-solver FRS
c     i_solver=2 : FTZ Tearing 
c     i_solver=3 : FTZ reversed shear 
         i_solver=1
         if(i_solver==2) i0p=1
...
c            call mywrite(Bp0,'Bp0=')
            call equi(size,r,p,dp,q,dq,Bz,Bp,k,0,q0,q_edge,beta,i_solver,r0,rs,m,n,is)
c$$$            i_solver=1
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     1           dqc,Bpc,rc,ic)
            ...
            alpha_DL = - dp(ic)*8.*rc/Bpc**2*k**2/dqc**2
            write(*,*) 'alpha = ',alpha_DL

            alpha_DL_array(2:size)=-8.*dp(2:size)*r(2:size)/Bp(2:size)**2*k**2/dq(2:size)**2
            call mysemilogyplot(r(2:size),alpha_DL_array(2:size),size-1,                         'alpha.cgm')
...
cccccccccccc without iend study cccccccccccccccc
     ...
     1           ,ifail,ic,iendleft,iendright,istartleft,prec,prec_left,tol,erl1,ers1,irestart)
...
cccccccccccc GOOD LAST VALUES cccccccc
cccccccccccccccccccccccccccccccccccccc
c beta=0.05
...
c beta=0.005
c            d_right = -958.4516064637208
c            d_left = 964.0376488886228

c FTZ Tearing
c            d_right = 16.19977868733851
c            d_left = 9.022767300587514
...
c       call logspace(0.5e-8,1.e-3,eta_array,size_plot)
       ...
       call logspace(1.e-8,2.5e-5,eta_array2,size_plot)


c      call linspace(0.101,0.9,gamma_dless_array,size_plot)
c      eta=1.e-6
...
c$$$      write(*,*) '-----------------------------------'
c$$$      write(*,*) 'eta =',eta
c$$$c      write(*,*) 'alpha=',alpha
c$$$c      call C05AJF(gamma_dless,0.0001,0.0,det,500,ifail)
c$$$c      write(*,*) '--- biss ---'
c$$$c$$$      if (j==1) gamlim=0.2
c$$$c$$$      if (j==2) gamlim=0.07
c$$$c$$$      if (j==3) gamlim=0.03
c$$$c$$$      if (j==4) gamlim=0.02
c$$$c$$$      if (j==5) gamlim=0.015
c$$$
c$$$
c$$$ccccc for beta = 0.01
c$$$c      gamlim=0.35
c$$$ccccc for beta= 0.1
c$$$c      gamlim=0.75
c$$$cccc for beta=0.001 and eta<=1.e-8
c$$$c$$$      gamlim=0.09
c$$$c$$$      call C05ADF(1.e-3,gamlim,1.e-15,0.,det,gamma_dless,ifail)
c$$$ccccc for beta=0.001
c$$$
c$$$      call C05ADF(0.101,0.6,1.e-15,0.,det,gamma_dless,ifail)
c$$$
c$$$      gamma=gammar*gamma_dless
c$$$      write(*,*) 'gamma dimensionless =',gamma_dless
c$$$      write(*,*) 'gamma =',gamma
c$$$
c$$$      gamma_dless_array(i)=gamma_dless
c$$$      gamma_array(i)=gamma

      eta=eta_array2(i)
     
      gammar=(eta*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.)
      gammar_array2(i)=gammar
...
c$$$      Qmax=4.8e-3
      ...
      Qmax=0.4
      rmatch=8
      call C05ADF(Qmin,Qmax,1.e-15,0.,det,gamma_dless,ifail)
      call innerc(cmplx(gamma_dless),E,F,G,H,K0,do,de,ifail,epsd
     1     ,rmatch,1,ye,yo,xary)
...
c      yo=yo/real(do)
      co=1.
      ce= - (d_right - real(do*Lr_DI))/(d_right-real(de*Lr_DI))*co
      write(*,*) 'cleft',ce-co
      write(*,*) 'cright',ce+co

      write(*,*) 'ce',ce
      ce=  (d_left - real(do*Lr_DI))/(d_left-real(de*Lr_DI))*co
      write(*,*) 'ce',ce

c      write(*,*) 'cright',ce+co

      

            open(unit=20,file='inright.txt')
      do 108 ii=1,1610
         write(20,*) co*real(yo(ii,2))+ce*real(ye(ii,2))
   
 108               continue


            open(unit=21,file='xary.txt')
      do 109 ii=1,1610
         write(21,*) real(xary(ii)*Lr)

 109  continue


            open(unit=22,file='inleft.txt')
      do 110 ii=1,1610
         write(22,*) ce*real(ye(ii,2))-co*real(yo(ii,2))
 ...
 110  continue   
      ...
      gamma_dless_array2(i)=gamma_dless
      gamma_array2(i)=gamma
      write(*,*) 'gamma dimensionless =',gamma_dless
      write(*,*) 'gamma =',gamma
      stop


cccccccccc Accuracy AM ccccccccccccccccc
      psi1l=(0.5*(1.-2.*sigma_l))*(-gamma_dless**2*sigma_l*(sigma_l-1)-delta0l*(E+F-H*(sigma_l-2)) )
      psi1s=(0.5*(1.-2.*sigma_s))*(-gamma_dless**2*sigma_s*(sigma_s-1)-delta0s*(E+F-H*(sigma_s-2)) )

      delta0l=-gamma_dless**2*(G+K*F)+gamma_dless**2*(G-K*E)-gamma_dless**2*K*H*(sigma_l+1)
      delta0s=-gamma_dless**2*(G+K*F)+gamma_dless**2*(G-K*E)-gamma_dless**2*K*H*(sigma_s+1)

      eps1l=psi1l+(H+gamma_dless)*(sigma_l+1)+(E+F)/gamma_dless
      eps1s=psi1s+(H+gamma_dless)*(sigma_s+1)+(E+F)/gamma_dless
      
      accl(i)=Lr*abs(eps1l)**(0.5)*erl1
      accs(i)=Lr*abs(eps1s)**(0.5)*ers1

ccccccccccccccccccccccccccccccccccccc
 30   continue

 
c$$$      call myplot(gamma_dless_array,det_array/maxval(abs(det_array)),size_plot                 ,'det_beta-3.cgm')
...
c      write(*,*) 'eta =',eta
c      write(*,*) 'log(resistivity) =',log(eta*ta_array(j))
c      write(*,*) 'ln(gamma) =',log(gamma)
      write(*,*) 'd_right =',d_right
      write(*,*) 'd_left =',d_left
            write(*,*) 'DI =',DI
c       write(*,*) 'DIbis =',DIbis
c       write(*,*) 'ta =',ta_array(j)
       write(*,*) 'order =',order
       write(*,*) 'lambda =',-2.*sqrt(-DI)+2*order-2
       write(*,*) 'Dr =',E+F
       pi=3.14159265358979323846264338327950288419716939937510
       gamfun34=1.2254167024651776451290983033628905268512392481080706112301
       gamfun14=3.6256099082219083119306851558676720029951676828800654674333
       Dr=E+F

       if (i0p==1) then

      open(unit=4,file='gamma_tear_C2.txt')
c$$$      do 67 i=1,size_plot
c$$$         write(4,*) (d_prime/(2*pi**2*rc**2*(1./k)/Lr_array(i)*gamfun34/gamfun14))**(4./5.)
c$$$     A*(eta_array(i)*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.)
c$$$ 67   continue
      do 78 i=1,size_plot
         write(4,*) (d_prime/(2*pi**2*rc**2*(1./k)/Lr_array2(i)*gamfun34/gamfun14))**(4./5.)
     A*(eta_array2(i)*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.)
 78         continue

      open(unit=1,file='eta_C2.txt')
c$$$      do 61 i=1,size_plot
c$$$         write(1,*) eta_array(i)
c$$$ 61   continue
      do 72 i=1,size_plot
         write(1,*) eta_array2(i)
 72         continue
            stop
         end if


c$$$      do 50 i=1,size_plot
c$$$          a=2*pi**2*rc**2*(1./k)/Lr_array(i)*gamfun34/gamfun14
c$$$          b=a*pi*Dr/4.
c$$$          coeff(1)=a
c$$$          coeff(6)=-d_prime
c$$$          coeff(7)=-b
...
c$$$                   gamma_dless_GGJ_array(i)=sol(1,j)**4
c$$$                   if (solmark==0) then
c$$$                      solmark=1
c$$$                   else
...
c$$$ 51          continue
c$$$          
c$$$         call mywrite(gamma_array(i)/(gamma_dless_GGJ_array(i)*gammar_array(i)),'mygamma/GGJgamma=')
c$$$ 50      continue

      do 52 i=1,size_plot
          a=2*pi**2*rc**2*(1./k)/Lr_array2(i)*gamfun34/gamfun14
          b=a*pi*Dr/4.
          coeff(1)=a
          coeff(6)=-d_prime
          coeff(7)=-b
          solmark=0
          call C02AGF(coeff,6,.true.,sol,work,ifail)
          do 53 j=1,6
             if(sol(2,j)==0.) then
                if(sol(1,j).ge.0.) then
                   gamma_dless_GGJ_array2(i)=sol(1,j)**4
                   if (solmark==0) then
                      solmark=1
                   else
                      write(*,*) 'multiple real positive growth rate'
                      stop
                   end if
                end if
             end if
...
c         call mywrite(gamma_array(i)/(gamma_dless_GGJ_array(i)*gammar_array(i)),'mygamma/GGJgamma=')
 52   continue
      call myplot(eta_array2,accl,size_plot,'accl.cgm')
      call myplot(eta_array2,accs,size_plot,'accs.cgm')
c$$$      open(unit=1,file='eta_final.txt')
c$$$      do 55 i=1,size_plot
c$$$         write(1,*) eta_array2(i)
c$$$ 55   continuec$$$      do 55 i=1,size_plot
c$$$         write(1,*) eta_array2(i)
c$$$ 55   continue
c$$$
c$$$      open(unit=2,file='gamma_final.txt')
c$$$      do 56 i=1,size_plot
c$$$         write(2,*) gamma_array2(i)
c$$$ 56   continue
c$$$
c$$$      open(unit=3,file='gamma_GGJ_final.txt')
c$$$      do 57 i=1,size_plot
c$$$         write(3,*) gamma_dless_GGJ_array2(i)*gammar_array2(i)
c$$$ 57   continue

c.....initialize plotting
      call ncarcgm(1,'acc.cgm')
c....make plot
      call mapgll(minval(eta_array2),maxval(eta_array2),minval(accl),maxval(accl),.1,1.,.1,1.)

      call trace(eta_array2,accl,size_plot,-1,-1,0.,0.)

      call colora('green')

      call trace(eta_array2,accs,size_plot,-1,-1,0.,0.)
...
c.....finalize plot file
 ...
 56   continue
      ...
      open(unit=3,file='acc-.txt')
      do 57 i=1,size_plot
         write(3,*) accs(i)
 57   continue

c.....initialize plotting
      call ncarcgm(1,'gamma_eta.cgm')
c....make plot
      call mapgll(minval(eta_array2),maxval(eta_array2),minval(gamma_array2),1.e-2,.1,1.,.1,1.)
...
c      call trace(eta_array,gamma_array,size_plot,-1,-1,0.,0.)
      call trace(eta_array2,gamma_array2,size_plot,-1,-1,0.,0.)
cccccccccc equation 90 of GGJ (approx of eq 88 of GGJ at small eta)
      call colora('green')
c      call trace(eta_array,((pi*(E+F)/4.)**(2./3.)*(eta_array*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.)) ,size_plot,-1,-1,0.,0.)
      call trace(eta_array2,((pi*(E+F)/4.)**(2./3.)*(eta_array2*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.)) ,size_plot,-1,-1,0.,0.)
cccccccccc equation 93 of GGJ
      if(d_prime.gt.0.) then
      call colora('blue')
c      call trace(eta_array,(d_prime/(2*pi**2*rc**2*(1./k)/Lr_array*gamfun34/gamfun14))**(4./5.)
c     A*(eta_array*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.) ,size_plot,-1,-1,0.,0.)
      call trace(eta_array2,(d_prime/(2*pi**2*rc**2*(1./k)/Lr_array2*gamfun34/gamfun14))**(4./5.)
     A *(eta_array2*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.) ,size_plot,-1,-1,0.,0.)
      endif

cccccccccc equation 88 of GGJ
      call colora('red')
c      call trace(eta_array,(gamma_dless_GGJ_array*gammar_array) ,size_plot,-1,-1,0.,0.)      
      call trace(eta_array2,(gamma_dless_GGJ_array2*gammar_array2) ,size_plot,-1,-1,0.,0.)   
 97   continue
c 98   continue 
...
c$$$     A (/4.65e-4,8.38e-4,12.18e-4,15.0075e-4,16.56e-4,23.335e-4,27.94e-4,31.83e-4,29.91e-4,25.71e-4 /),10,1,1,0.,0.)
      call pointc('*',(/1.e-6,2.e-6,3.e-6,5.e-6,7.5e-6/),
     A (/5.084e-4,8.2325e-4,10.1225e-4,13.30975e-4,15.989e-4 /),5,1,1,0.,0.)
      call frame(0)
c.....finalize plot file
      call plote
...
c$$$ 61   continue
 ...
 62   continue  
      ...
      open(unit=3,file='gamma_C2.txt')
c$$$      do 65 i=1,size_plot
c$$$         write(3,*) gamma_array(i)
c$$$ 65   continue
      do 66 i=1,size_plot
         write(3,*) gamma_array2(i)
 66   continue
    
      open(unit=2,file='gamma_inter_C2.txt')
c$$$      do 63 i=1,size_plot
c$$$         write(2,*) ((pi*(E+F)/4.)**(2./3.)*(eta_array(i)*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.))
c$$$ 63   continue
      do 64 i=1,size_plot
         write(2,*) ((pi*(E+F)/4.)**(2./3.)*(eta_array2(i)*real(n)**2*Bpc**2*dqc**2/(rho*rc**2))**(1./3.))
 64   continue
...
c$$$ 67   continue
 ...
 68   continue
      ...
      open(unit=5,file='gamma_an_C2.txt')
c$$$      do 69 i=1,size_plot
c$$$         write(5,*) gamma_dless_GGJ_array(i)*gammar_array(i)
c$$$ 69   continue
      do 70 i=1,size_plot
         write(5,*) gamma_dless_GGJ_array2(i)*gammar_array2(i)
 70      continue
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     1     d_right,d_left,DI,ifail,icb,iendleft,iendright,istartleft,prec,precleft,tol,erl1,ers1,irestart)
      implicit none
      real D02EJW,D02EJX,D02EJY
      external D02EJW,D02EJX,D02EJY
      integer size,m,n,order,ic,ifail,fact,icb
      real, dimension(1000001):: r,p,q,dp,dq,Bz,Bp,fr,gr,rb,grbis
...
c Suydam criterion must be fulfilled only at left endpoints ==> not necessary to check that      
c$$$      if(-dp(size)-r(size)/8.*Bz(size)**2*(dq(size)/q(size))**2 .ge. 0) then
c$$$         write(*,*) 'Suydam criterion not satisfied at the edge'
c$$$         stop
c$$$      end if
...
c     fact=fact*i
         ...
         write(*,*) 'er_l_right =',er_l(i)*(matchright)**(sigma_l+i)
         write(*,*) 'er_s_right =',er_s(i)*(matchright)**(sigma_s+i)
         write(*,*) 'er_l_left =',er_l(i)*(-1.)**i*(matchleft)**(sigma_l+i)
         write(*,*) 'er_s_left =',er_s(i)*(-1.)**i*(matchleft)**(sigma_s+i)

         write(*,*) 'er_l_right_conv =',er_l(i)*(matchright)**(sigma_l+i)/er_l_right
         write(*,*) 'er_s_right_conv =',er_s(i)*(matchright)**(sigma_s+i)/ er_s_right
         write(*,*) 'er_l_left_conv =',er_l(i)*(-1.)**i*(matchleft)**(sigma_l+i)/er_l_left
         write(*,*) 'er_s_left_conv =',er_s(i)*(-1.)**i*(matchleft)**(sigma_s+i)/er_s_left
 ...
 17   continue
      ...
      do 16 i=1,3
         write(*,*) 'i=',i
         write(*,*) 'ksi_l_right',ksi_l(i)*(matchright)**i
         write(*,*) 'ksi_s_right',ksi_s(i)*(matchright)**i
         write(*,*) 'ksi_l_left',ksi_l(i)*(-matchleft)**i
         write(*,*) 'ksi_s_left',ksi_s(i)*(-matchleft)**i
         if(i.ne.1) then
            write(*,*) 'ksi_l_right_conv',ksi_l(i)*(matchright)**i/ksi_l_right
            write(*,*) 'ksi_s_right_conv',ksi_s(i)*(matchright)**i/ ksi_s_right
            write(*,*) 'ksi_l_left_conv',ksi_l(i)*(-matchleft)**i/ksi_l_left
            write(*,*) 'ksi_s_left_conv',ksi_s(i)*(-matchleft)**i/ksi_s_left
         end if
         ksi_l_right=ksi_l_right+ksi_l(i)*(matchright)**i
         ksi_s_right=ksi_s_right+ksi_s(i)*(matchright)**i
 ...
 16   continue
      ...
      der_s_right=ksi_s_right/fr(ic+iendright)
      der_l_right=ksi_l_right/fr(ic+iendright)
      der_s_left=ksi_s_left/fr(ic-iendleft)
      der_l_left=ksi_l_left/fr(ic-iendleft)
      endif
...
ccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      f2=dfrc(2)
      f3=dfrc(3)
      er_s(1) = dgrc(1) / 2. / dfrc(2)
      er_s(2) = 1. / 6. / dfrc(2) * (dgrc(2) - dgrc(1) / 2. / dfrc(2) * (3. *dfrc(3) - dgrc(1)))
      ksi_s(1)=0.
      ksi_s(2)=0.5*dgrc(1)
      ksi_s(3)=1./3. * (dgrc(2)+0.5*dgrc(1)**2/dfrc(2))
      c1=f3/f2 + 2. * er_s(1)
      c2=dfrc(4)/dfrc(2)+er_s(1)**2+2.*er_s(2)+2.*f3/f2*er_s(1)-c1**2
      if (irestart==1)  return
...
c     fact=fact*i
 ...
 21   continue
      ...
      write(*,*) 'er_l_right=',er_l_right
      write(*,*) 'er_s_right=',er_s_right

      write(*,*) 'er_l_left=',er_l_left
      write(*,*) 'er_s_left=',er_s_left


      
      write(*,*) 'der_l_right=',der_l_right
      write(*,*) 'der_s_right=',der_s_right

      write(*,*) 'der_l_left=',der_l_left
      write(*,*) 'der_s_left=',der_s_left
...
ccccccccccccccccccccccccccccccccccccccccc
      ...
      d_right=(zright*der_l_right-er_l_right)/(er_s_right-zright*der_s_right)
      write(*,*) 'cright_RK =',vec_r0(1)/(er_l_right+d_right*er_s_right)

      write(*,*) 'zright*der_l_right=',zright*der_l_right
      write(*,*) 'zright*der_s_right=',zright*der_s_right
      write(*,*) 'numerator=',(zright*der_l_right-er_l_right)
      write(*,*) 'denominator=',(er_s_right-zright*der_s_right)
      d_left=(zleft*der_l_left-er_l_left)/(er_s_left-zleft*der_s_left)
      write(*,*) 'cleft_RK =',vec_l0(1)/(er_l_left+d_left*er_s_left)
      ...
      write(*,*) 'd_left =',d_left
      write(*,*) 'W0right',er_l_right*fr(ic+iendright)*der_s_right-fr(ic+iendright)*der_l_right*er_s_right


      write(*,*) 'W0left',er_l_left*fr(ic-iendleft)*der_s_left-fr(ic-iendleft)*der_l_left*er_s_left


      write(*,*) 'Wexact',(2.*sigma_s+1.)*dfrc(2)
      prec=abs(er_l_right*fr(ic+iendright)*der_s_right-fr(ic+iendright)*der_l_right*er_s_right-(2.*sigma_s+1.)*dfrc(2))/abs((2.*sigma_s+1.)*dfrc(2))
      precleft=abs(er_l_left*fr(ic-iendleft)*der_s_left-fr(ic-iendleft)*der_l_left*er_s_left+(2.*sigma_s+1.)*dfrc(2))/abs((2.*sigma_s+1.)*dfrc(2))
c     write(*,*) 'd_right2',
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real,dimension(1000001) ::r,fr,gr,rb,frb,rbb,rbb2
      common /fg_block/ fr,gr,rc,ic,rb
c$$$      step=1./real(size-1)
c$$$      r=rb
c$$$c     supposes a=1
c$$$      if (abs(x-rc).le.step*0.01) then
c$$$         funfr=fr(ic) 
c$$$c     write(*,*) 'i=0'
c$$$         return
c$$$      end if
c$$$
c$$$      do 10 i=1,10
c$$$         if(abs(x-(rc+(2*i-1)*step)).le.step*0.01) then 
c$$$            funfr=fr(ic+(2*i-1))
c$$$c     write(*,*) 'i_plus =',i
c$$$            return
c$$$         end if
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      common /size_block/ size
      integer, dimension(1) ::icb
      real,dimension(1000001) ::r,fr,gr,rb,grb,rbb,rbb2
      common /fg_block/ fr,gr,rc,ic,rb
c$$$      step=1./real(size-1)
c$$$      r=rb
c$$$c     supposes a=1
...
c$$$         if(abs(x-(rc+(2*i-1)*step)).le.step*0.01) then 
c$$$            fungr=gr(ic+(2*i-1))
c$$$c     write(*,*) 'i_plus_g =',i
c$$$            return
c$$$         end if
c$$$
c$$$         if(abs(x-(rc-(2*i-1)*step)).le.step*0.01) then 
c$$$            fungr=gr(ic-(2*i-1))
c$$$c     write(*,*) 'fungr =',fungr
c$$$c     write(*,*) 'i_minus_g =',i
c$$$            return
c$$$         end if
c$$$
c$$$c     if(abs(x-(rc-(2*i-1)*step)).le.step*0.01)fungr=gr(ic-(2*i-1))
c$$$ 10   continue
c$$$      write(*,*) 'oups:',x
...
c     stop
      ...
      call innerc(cmplx(gamma),E,F,G,H,K0,do,de,ifail,epsd
     1     ,rmatch,0,ye,yo,xary)
      if (ifail.ne.0)then
