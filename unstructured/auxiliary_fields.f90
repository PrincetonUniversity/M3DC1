...
if (ispradapt .eq. 1) then
  ...
  if(ikprad.ge.1) call create_field(kprad_totden)
  if(jadv.eq.0) then
...
subroutine destroy_auxiliary_fields
  ...
  if(ikprad.ge.1) call destroy_field(kprad_totden)
  if(jadv.eq.0) then
...
!           dofs(i) = int1(temp79f)
  ...
  if(ikprad.ge.1) call newvar_solve(kprad_totden%vec, mass_mat_lhs)
  if(jadv.eq.0) then
