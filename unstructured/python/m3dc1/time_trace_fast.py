...
def plot_pbdiagram(ntor,rotation=None,eta=1.0,flmodel='2f',gyro=1,path='./',excl=[]):
    #ntor: toroidal mode number
    #rotation: 0 - no rotation, 1 - rotation, 2 - rotation without centrifugal effects, 3 - mixed full rotation and no centrifugal effects
    #eta
    
    files = glob.glob(path+"growth_rates*.dat")
    #print(files)
    offset = 0.0001
    ...
    for f in files:
        vpr.append(Gamma_file(f))
    
    for i in range(len(vpr)):
        if (vpr[i].eta == eta and vpr[i].fluidmodel == flmodel):# and vpr[i].rotation==rotation and vpr[i].bscale==bscale):
            if vpr[i].vpnum not in excl:
                if len(vpr[i].gsconvgd)>0:
                    slurm = True
                else:
                    slurm = False
                    fpyl.printwarn('WARNING: No data from Slurm file available.')
                
                n_ind = fpyl.get_ind_at_val(vpr[i].n_list,ntor,unique=True)
                if (slurm == True and vpr[i].gsconvgd[n_ind] == 0):
                    fpyl.printwarn('WARNING: Equilibrium for '+str(vpr[i].vpnum)+' is not converged. Varyped run '+str(vpr[i].vpnum)+' omitted from plot.')
                elif (slurm == True and vpr[i].finalerrgs[n_ind] >= 0.1):
                    fpyl.printwarn('WARNING: Final GS Err is large for '+str(vpr[i].vpnum)+'. Varyped run '+str(vpr[i].vpnum)+' omitted from plot.')
                else:
                    if (vpr[i].flat_list[n_ind] != 1 and vpr[i].gamma_manual_list[n_ind] != 1 and vpr[i].not_noisy_list[n_ind] != 1 and False): #ToDo: Remove False
                        x_bad.append(vpr[i].pped)
                        y_bad.append(vpr[i].jped)
                        vpnum_bad.append(vpr[i].vpnum)
                    else:
                        x_okay.append(vpr[i].pped)
                        y_okay.append(vpr[i].jped)
                        vpnum_okay.append(vpr[i].vpnum)
                
                        if (vpr[i].gamma_list[n_ind] < 0.0) or (vpr[i].flat_list[n_ind] != 1 and vpr[i].gamma_manual_list[n_ind] != 1 and vpr[i].not_noisy_list[n_ind] != 1): #ToDo: Remove last condition
                            gamma.append(0.0)
                        else:
                            gamma.append(vpr[i].gamma_list[n_ind])
                
    
    fig, ax = plt.subplots(num=100+ntor)
    plt.plot(x_okay,y_okay,lw=0,marker='.')
    if len(x_bad)>0:
        plt.plot(x_bad,y_bad,lw=0,marker='.',c='r')
    cont = plt.tricontourf(x_okay,y_okay,gamma,20)
    plt.tricontour(x_okay,y_okay,gamma,[0.01],colors=['magenta'],linewidths=1,zorder=10)
    
    for i, txt in enumerate(vpnum_okay):
        ax.annotate(txt, (x_okay[i]+offset, y_okay[i]+offset))
    for i, txt in enumerate(vpnum_bad):
        ax.annotate(txt, (x_bad[i]+offset, y_bad[i]+offset))
    
    cbar = fig.colorbar(cont,orientation='vertical',ax=ax)#,format=ticker.FuncFormatter(fpyl.fmt),pad=0.1
    ...
    ax.set_yticklabels([])
    ax.set_xticklabels([])
    plt.title('Growth rates for n='+str(ntor))
