...
int geqdsk_source::eval_scalar_field(const vector_field field, 
				    const double* x, double* val)
{
  switch(field) {

  default:
    return field_source::eval_scalar_field(field, x, val);
  }
}

int geqdsk_source::eval_vector_field(const vector_field field, 
				    const double* x, double* val)
{
  switch(field) {

  case(MAGNETIC_FIELD):
    return eval_magnetic_field(x, val);

  default:
    return field_source::eval_vector_field(field, x, val);
  }
}

int geqdsk_source::eval_scalar_field_grad(const vector_field field, 
					  const double* x, double* val)
{
  switch(field) {

  default:
    return field_source::eval_scalar_field_grad(field, x, val);
  }
}
