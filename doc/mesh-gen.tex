...
\begin{itemize}
\item numBdry: number of boundary files defined by peice-wise linear points given for the construction of the loops (default: 0). Each boundary file corresponds to a loop in PUMI.
\item bdryFile: For each boundary file, the user has to provide its file name followed by the unique loop ID and desired mesh size on the loop.
  ...
  \item For more than one boundary files (numBdry$>$1), the order of the boundary files can be in any order
  \end{itemize}
