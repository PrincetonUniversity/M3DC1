...
void f_dcreate_dist_matrix(fptr *A, int *m, int *n, int *nnz, double *nzval,
			   int *rowind, int *colptr, fptr *grid)
{
   dcreate_dist_matrix((SuperMatrix *) *A, (int_t) *m, (int_t) *n,
		       (int_t) *nnz,  (double *) nzval,
...
void printoffc_(long int *handle)
{
  ((superlu_options_t *) *handle)->PrintStat = NO;
}
